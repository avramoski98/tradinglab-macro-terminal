...
async function fetchBank(bank) {
  ...
  try {
    ...
    const parsed = parseProbabilities(text, bank.latest);
...
export default async function handler(req, res) {
  ...
  return res.status(200).json({
    mode: 'live',
    snapshot: new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Skopje', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(SOURCE_UPDATED_AT)),
